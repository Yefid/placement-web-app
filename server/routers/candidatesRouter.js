const express = require('express');
const router = express.Router();
const candidatesdbBL = require('../models/candidates/candidatesdbBL');
const { getRoleFromId, VerifcationStatus } = require('../helpers/auth');

//auth - checks token and role
const restrictto = async (req, res, next) => {
	var token = req.headers['x-access-token'];

	let respCallBack = async (resp) => {
		const resp2 = await resp;
		if (resp2.auth) {
			res.role = resp2.userRole;
			next();
		} else {
			res.status(resp2.status).send({ userRole: resp2.userRole });
		}
	};

	await VerifcationStatus(token, respCallBack);
};

router.use(restrictto);

router.route('/limited').get(async (req, resp) => {
	let data = {};
	['Admin', 'BD'].includes(resp.role)
		? (data = await candidatesdbBL.getLimitedCandidates())
		: (data = { status: 401, message: 'unauthorized' });

	return resp.json(data);
});

router.route('/').get(async (req, resp) => {
	let data = {};
	['Admin', 'BD'].includes(resp.role)
		? (data = await candidatesdbBL.getAllCandidates())
		: (data = { status: 401, message: 'unauthorized' });

	return resp.json(data);
});

router.route('/addCandidateFromMail').post(async (req, resp) => {
	let status = {};
	['Admin', 'BD'].includes(resp.role)
		? (status = await candidatesdbBL.addCandidatesFromMail(req.body))
		: (status = { status: 401, message: 'unauthorized' });

	return resp.json(status);
});

router.route('/:id').get(async (req, resp) => {
	let data = {};
	['Admin', 'BD'].includes(resp.role)
		? (data = await candidatesdbBL.getCandidate(req.params.id))
		: (data = { status: 401, message: 'unauthorized' });

	return resp.json(data);
});

router.route('/').post(async (req, resp) => {
	let status = {};
	['Admin', 'BD'].includes(resp.role)
		? (status = await candidatesdbBL.addCandidate(req.body))
		: (status = { status: 401, message: 'unauthorized' });

	return resp.json(status);
});

router.route('/:id').put(async (req, resp) => {
	let status = {};
	['Admin', 'BD'].includes(resp.role)
		? (status = await candidatesdbBL.updateCandidate(req.params.id, req.body))
		: (status = { status: 401, message: 'unauthorized' });

	return resp.json(status);
});

router.route('/:id').delete(async (req, resp) => {
	let status = {};
	['Admin', 'BD'].includes(resp.role)
		? (status = await candidatesdbBL.deleteCandidate(req.params.id))
		: (status = { status: 401, message: 'unauthorized' });

	return resp.json(status);
});

module.exports = router;
